...
$r = new Redis();
$r->pconnect('127.0.0.1', 6379);                 // persistent, reused per worker thread
$hits = 0;
for ($k = 0; $k < F; $k++) {                      // sequential calls, NOT pipelined (like a real page)
    $row = $r->hGetAll('row:'.id_for(mt_rand(0, N-1)));
    if ($row) $hits++;
}
echo $hits;

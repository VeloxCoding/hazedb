...
// param numbering, auto-PK per row, and the all-or-nothing atomicity of the
// batch (it commits as one transaction).
...
func TestInsertRejectsColumnRef(t *testing.T) {
	db := openMem(t)
	cases := []string{
		"INSERT INTO users (name, age) VALUES (name, ?)",        // bare colRef
		"INSERT INTO users (name, age) VALUES (?, age + ?)",     // colRef in arithmetic
		"INSERT INTO users (name, age) VALUES (nope, ?)",        // unknown-column colRef
		"INSERT INTO users (name, age) VALUES (?, ?), (?, age)", // multi-row, second tuple
	}
...
func TestInsertRejectsDuplicateColumn(t *testing.T) {
	db := openMem(t)
	if _, err := db.Exec("INSERT INTO users (name, name) VALUES (?, ?)"); !errors.Is(err, ErrParse) {

...
func TestWideUpdateWALRoundTrip(t *testing.T) {
	...
	db, err := Open(Options{Schema: Schema{}, WALPath: path, drainInterval: -1})
	if err != nil {
		t.Fatal(err)
	}
	...
	db2, err := Open(Options{Schema: Schema{}, WALPath: path, drainInterval: -1})
	if err != nil {
...
func TestRejectedDuplicateInsertDoesNotCorruptWAL(t *testing.T) {
	...
	db2, err := Open(Options{Schema: testSchema(), WALPath: path, drainInterval: -1})
	if err != nil {
...
func TestWALRejectsForeignVersion(t *testing.T) {
	...
	if _, err := Open(Options{Schema: testSchema(), WALPath: path, drainInterval: -1}); !errors.Is(err, ErrWALVersion) {
...
func TestRecoverySkipsCorruptSuffix(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "test.wal")
	db, err := Open(Options{Schema: testSchema(), WALPath: path, walFlushInterval: -1, drainInterval: -1})
	if err != nil {
		t.Fatal(err)
	}
	for i := 1; i <= 5; i++ {
	...
	db2, err := Open(Options{Schema: testSchema(), WALPath: path, walFlushInterval: -1, drainInterval: -1})
	if err != nil {

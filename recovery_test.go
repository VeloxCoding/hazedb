...
func TestWALRejectsForeignVersion(t *testing.T) {
	...
	if _, err := Open(Options{Schema: testSchema(), WALPath: path, CompanionPath: ":memory:"}); !errors.Is(err, ErrWALVersion) {
...
func TestRecoverySkipsCorruptSuffix(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "test.wal")
	db, err := Open(Options{Schema: testSchema(), WALPath: path, walFlushInterval: -1, CompanionPath: ":memory:"})
	if err != nil {
		t.Fatal(err)
	}
	for i := 1; i <= 5; i++ {
	...
	db2, err := Open(Options{Schema: testSchema(), WALPath: path, walFlushInterval: -1, CompanionPath: ":memory:"})
	if err != nil {

...
**Updated:** 2026-06-10 (rev. 31 — atomic batches (multi-row `INSERT` / transactions) capped at 1000 mutations (`maxTxnMutations`): bounds shard-lock hold + memory and keeps the count under the `recTxn` uint16 count field, which previously wrapped silently above 65535; over the cap is a clean `ErrBatchTooLarge`)

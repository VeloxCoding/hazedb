...
**Updated:** 2026-06-12 (rev. 34 — the parser bounds expression nesting (`maxExprDepth` = 256), closing a stack-overflow remote-kill DoS: a crafted deeply-nested `WHERE` drove the recursive-descent parser past the goroutine stack limit, a fatal error `recover()` cannot catch. rev. 33 documented the `MaxBytes` byte cap + `/meta` store stats — every insert reserves its byte cost against a store-wide budget and is rejected with `ErrCapacity` / HTTP 507 over the cap; the store never auto-evicts. rev. 32 — `walVersion` 1→2 and replay now rejects ANY non-current version)

...
Durability across restart rests on two on-disk artefacts: the **WAL segments** (the recent write tail) and the **SQLite mirror** (a compacted base). The mirror is the SQLite companion *file* — present whenever WAL is on, at `<WALPath>/hazedb.db` (override with `Options.CompanionPath`). It is always a real file; there is no in-memory companion. With no WAL there is no companion and nothing on disk to recover from.
...
**No WAL means no companion.** Without WAL the store is pure in-memory — nothing on disk, no mirror, no recovery (the source of truth lives elsewhere, so a restart starts empty). The companion file and its mirror exist only alongside the WAL.

**The companion is a file, only with WAL.** When WAL is on, the companion file (`<WALPath>/hazedb.db`, or `CompanionPath`) is the data mirror's home and the recovery base — always a real file, never in-memory. Beyond the mirror it holds the `_hz_events` table (`ts, level, kind, message`), where rare operator-relevant events — a corrupt segment skipped, a drain failure — are recorded as queryable rows for a dashboard; with no WAL (no companion) these go to the standard log instead. A user `CREATE TABLE` in the reserved `_hz_` namespace is rejected so it cannot collide. Metrics and per-request counters stay in memory (the stats endpoints); high-frequency occurrences are counted, not logged per event.

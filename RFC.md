...
**Arena slots are not reclaimed in a running process** (known limitation, partially mitigated). A delete nils the arena slot and drops the PK from the pkMap/pkDirectory, but the slot itself is never reused — only a restart-from-checkpoint compacts the arena. So a heavy insert+delete workload grows arena memory until that restart; `/meta`'s `tombstones` / (`rows` + `tombstones`) is the visibility into it.
...
Durability across restart rests on two on-disk artefacts: the **WAL segments** (the recent write tail) and the **SQLite mirror** (a compacted base). The mirror is the data half of the always-present SQLite companion (`Options.CompanionPath`; in-memory when empty): it is active when WAL is on AND the companion is a persistent file. WAL on with no companion path is WAL-only — the WAL alone is the recovery source.
...
**Recovery without a mirror.** WAL-only — WAL on with no companion path — uses the WAL itself as the recovery source: every segment replays into memory in order. This is native-fast (the typed-mutation decoder, no SQLite layer) but **uncompacted** — replay cost scales with total write history, not current size — so it is bounded only by how much WAL is retained.

**The companion is always present.** Every mode opens the SQLite companion. Left unset, `CompanionPath` defaults to a file `hazedb.db` inside `WALPath` when WAL is on (the data mirror + events persist and it is the recovery base), and to `hazedb.db` in the working directory when there is no WAL (a memory-only store keeps its rows in RAM, but the events log still persists); pass a path, or `":memory:"`, to override. Beyond the data mirror it holds operational data that must not depend on durability: the `_hz_events` table (`ts, level, kind, message`), where rare operator-relevant events — a corrupt segment skipped, a drain failure — are recorded as queryable rows for a dashboard. A user `CREATE TABLE` in the reserved `_hz_` namespace is rejected so it cannot collide. Metrics and per-request counters stay in memory (the stats endpoints); high-frequency occurrences are counted, not logged per event.
...
**WAL bounding — resolved by the SQLite mirror (M7).** With a mirror configured (WAL on + a persistent `CompanionPath`), the drain deletes each segment once it is durably in SQLite, so the on-disk WAL is bounded to the **undrained tail**, and recovery rebuilds from the mirror base then replays only that tail — cold-start scales with live data size, not total writes ever made. **Without a mirror** (no companion path, WAL-only mode) the original caveat still holds: the WAL grows without bound for the life of the process and restart replays the entire history from the beginning, so recovery time grows linearly with total writes. For a long-running, write-heavy deployment, configure the mirror; the WAL-only mode is for ephemeral or low-churn use where a bounded WAL is not needed.
...
| WAL truncation / recovery time | § SQLite mirror bounds both: drained segments deleted, base + tail recovery; WAL-only mode still unbounded | *Recovery*, *WAL bounding* |

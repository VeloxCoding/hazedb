...
**Updated:** 2026-06-10 (rev. 30 — multi-row `INSERT`: `VALUES` accepts >1 tuple, committed atomically via the transaction path (one `recTxn` envelope); amortises per-row WAL overhead, ~96× per-row under `WALSyncPerWrite` where the batch fsyncs once instead of per row)

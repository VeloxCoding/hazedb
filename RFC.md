...
**Updated:** 2026-06-11 (rev. 32 — `walVersion` 1→2 and replay now rejects ANY non-current version (was: only newer). The rev. 29 nsets u8→u16 change altered the opUpdate byte layout without a bump, so a pre-change WAL (undrained crash-WAL) would have misparsed under the same version byte; the bump makes such a WAL fail loud instead. A format change must bump the version going forward)
